...
export class InfluenceMap {
  ...
  update(frame: Frame) {
    ...
    for (let [p, prevValue] of bufferDataTiles) {
      ...
      if (Math.abs(mostDominant) < Math.abs(baseValue)) {
        this.dataTiles.set(p, baseValue);
      } else {
